    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          fetch(
            `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${env.REACT_APP_GOOGLE_MAPS_API_KEY}`
          )
            ...
            .then((data) => {
              if (data.results && data.results.length > 0) {
                ...
                fetch(
                  `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lng}&units=metric&appid=${env.REACT_APP_WEATHER_API_KEY}`
                )
                  .then((response) => response.json())
                  .then((weatherData) => {
