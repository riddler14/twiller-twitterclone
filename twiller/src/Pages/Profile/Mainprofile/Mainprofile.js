...
const Mainprofile = ({ user }) => {
  ...
  const [accessories, setAccessories] = useState(
    seedParameters[currentSeed].accessories[0]
  );
  const [hairColor, setHairColor] = useState(
    seedParameters[currentSeed].hairColor[0]
  );
  const [clothing, setClothing] = useState(
    seedParameters[currentSeed].clothing[0]
  );
  const [skinColor, setSkinColor] = useState(
    seedParameters[currentSeed].skinColor[0]
  );
  const [backgroundColor, setBackground] = useState(
    seedParameters[currentSeed].backgroundColor[0]
  );
  ...
  const handlePostDelete = (deletedPostId) => {
    ...
    setpost((prevPosts) => prevPosts.filter((p) => p._id !== deletedPostId));
  ...
  const fetchLoginHistory = async () => {
    try {
      const response = await axios.get(
        "https://twiller-twitterclone-2-q41v.onrender.com/login-history",
        {
  ...
  const toggleNotificationPreference = async () => {
    try {
      ...
      const response = await axios.patch(
        `https://twiller-twitterclone-2-q41v.onrender.com/update-notification-preference/${userEmail}`,
        {
  ...
  const generateAvatarUrl = () => {
    ...
    if (seedParameters[currentSeed].accessories)
      params.push(`accessories=${accessories}`);
    if (seedParameters[currentSeed].hairColor)
      params.push(`hairColor=${hairColor}`);
    if (seedParameters[currentSeed].clothing)
      params.push(`clothing=${clothing}`);
    if (seedParameters[currentSeed].skinColor)
      params.push(`skinColor=${skinColor}`);
    if (seedParameters[currentSeed].backgroundColor)
      params.push(`backgroundColor=${backgroundColor}`);
    ...
    const fetchSubscriptionPlan = async () => {
      try {
        const response = await fetch(
          `https://twiller-twitterclone-2-q41v.onrender.com/subscription?email=${encodeURIComponent(
            user.email
          )}`
        );
              ...
              {showLoginHistoryPopup && (
                <div className="login-history-popup">
                  <div className="popup-content">
                      ...
                      <ul>
                        {loginHistory.map((entry, index) => (
                          <li key={index}>
                            ...
                            <br />
                            IP: {entry.ip}, Browser: {entry.browser}, OS:{" "}
                            {entry.os}, Device: {entry.device}
                          </li>
                        ))}
                      </ul>
                    ) : (
                    ...
                    <button onClick={() => setShowLoginHistoryPopup(false)}>
                      {t("Close")}
                    </button>
                  </div>
                </div>
